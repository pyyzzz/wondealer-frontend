...
const AdminApi = {
  ...
  getMembers: (params) => AxiosInstance.get("/api/admin/members", { params }),
  ...
  // ── 상품 관리 ──────────────────────────────────────────────
  // GET /api/admin/items — 전체 상품 목록 (AdminController에는 이미 있었는데
  // 프론트 래퍼에 빠져 있어서 ItemsTab이 호출 시 에러가 났던 부분)
  getItems: (params) => AxiosInstance.get("/api/admin/items", { params }),

  // DELETE /api/admin/items/:itemId — 상품 강제 삭제
  deleteItem: (itemId) => AxiosInstance.delete(`/api/admin/items/${itemId}`),

  // ── 게임 관리 ──────────────────────────────────────────────
  // POST /api/admin/games — 게임 추가
  createGame: (data) => AxiosInstance.post("/api/admin/games", data),
