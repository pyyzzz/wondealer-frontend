...
const PaymentPage = ({
  ...
  const productPrice = Number(
    product?.price ?? product?.basePrice ?? product?.itemPrice ?? product?.tradePrice ?? 0,
  );
  const serviceFee = 0;
  ...
  const handleProcessPayment = async () => {
    ...
    try {
      ...
      if (!productPrice) {
        throw new Error("상품 금액 정보를 찾을 수 없어 결제를 진행할 수 없습니다.");
      }
      ...
      if (paymentMethod === "WONPAY") {
        ...
        if (!PortOne) {
          throw new Error("결제 모듈이 아직 로드되지 않았습니다. 잠시 후 다시 시도해 주세요.");
        }
        ...
        if (onPaymentSuccess) onPaymentSuccess(completeData?.balance ?? balance);
        onClose();
      }
    } catch (error) {
          ...
          <ReceiptCard>
            ...
            <TermsList>
              <TermsLabel>
                ...
                />
                <TermsText>
                  <b style={{ color: "var(--text-secondary)", fontWeight: "600" }}>(필수)</b>{" "}
              ...
              <TermsLabel>
                ...
                />
                <TermsText>
                  <b style={{ color: "var(--text-secondary)", fontWeight: "600" }}>(선택)</b>{" "}
                  이벤트 및 혜택 알림 수신 동의
                </TermsText>
              </TermsLabel>
            </TermsList>
...
const PresetButton = styled.button`
  ...
  background: ${(props) =>
    props.$isActive
      ...
      : "var(--bg-container)"};
  border: 2px solid
    ${(props) => (props.$isActive ? "var(--color-primary)" : "var(--outline-variant)")};
