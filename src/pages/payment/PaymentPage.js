  ...
  const handleProcessPayment = async () => {
    ...
    try {
      ...
      if (paymentMethod === "WONPAY") {
        ...
        const tradeRes = await TradeApi.createWonPayTrade(itemId);
        const tradeData = tradeRes.data?.data ?? tradeRes.data;
        const newTradeId =
          tradeData?.tradeId ??
          tradeData?.id ??
          tradeData?.trade?.tradeId ??
          tradeData?.trade?.id ??
          tradeData?.payment?.tradeId ??
          null;
        ...
        const newTradeId =
          completeData?.tradeId ??
          completeData?.id ??
          completeData?.trade?.tradeId ??
          completeData?.trade?.id ??
          completeData?.payment?.tradeId ??
          null;

        if (onPaymentSuccess)
          onPaymentSuccess({
            tradeId: newTradeId,
            balance: completeData?.balance ?? balance,
          });
        onClose();
      }
    } catch (error) {
