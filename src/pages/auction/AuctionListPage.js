...
const isAuctionEnded = (auction) => {
  ...
  if (
    auctionId != null &&
    localStorage.getItem(`wondealerAuctionSettled:${auctionId}`) === "true"
  ) {
    return true;
  }
  ...
  useEffect(() => {
    const calc = () => {
      if (isAuctionEnded(auction)) {
        return { text: "경매 종료", urgent: false };
      }
      const endTimeStr = auction?.endTime ?? auction?.endAt;
