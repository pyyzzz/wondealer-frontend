...
export default function ItemDetailPage() {
  ...
  const handleBuy = async () => {
    ...
    if (
      !window.confirm(
        `${fmt(item.price ?? item.basePrice)}원에 구매하시겠습니까?`,
      )
    )
      return;
    ...
    try {
      ...
      alert("구매가 완료되었습니다! 마이페이지에서 거래 내역을 확인하세요.");
      navigate("/mypage");
    } catch (err) {
      setError(
        ...
        <InfoSection>
          ...
          <MetaGrid>
            <MetaItem>
              ...
              <MetaVal>{item.gameServer ?? item.serverName ?? "-"}</MetaVal>
            </MetaItem>
            <MetaItem>
              <MetaLabel>서버</MetaLabel>
              <MetaVal>{item.gameServer ?? item.serverName ?? "-"}</MetaVal>
            ...
            <MetaItem>
              ...
              <MetaVal>{item.status ?? "판매 중"}</MetaVal>
            </MetaItem>
          </MetaGrid>
          ...
          <SellerBox>
            ...
            <SellerInfo>
              <SellerName>
                {item.seller ?? item.sellerNickname ?? "판매자"}
              </SellerName>
              <SellerMeta>
          ...
          <PriceSection>
            ...
            <PriceRow>
              <PriceMain>
                {fmt(price)} <PriceCurrency>KRW</PriceCurrency>
              </PriceMain>
              <PriceChange>
